...
for (const mission of ['luxeuil', 'tehran'])
    for (const mobile of [false, true]) {
        test(`${mission} audio playback and settings ${mobile ? 'mobile' : 'desktop'}`, async ({
            ...
            await page.goto(`/3d-plane/?mission=${mission}`);
            ...
            await page.screenshot({
                path: info.outputPath('audio-settings.png')
            });
...
for (const aircraft of ['mirage', 'phantom', 'cessna'])
    test(`${aircraft} effect voices respect weapons and dispose cleanly`, async ({
        page
    }) => {
