...
Run from the repository root: python3 scripts/scenery/upgrade-surface-cache.py
Unrelated features, credits and snapshot dates remain unchanged.
"""
import gzip, json, math
from pathlib import Path
from scenery_metadata import surface_metadata
for map_path, cache in [(Path('data/saint-cyr/map.json'), Path('data/saint-cyr/cache')), (Path('data/luxeuil/map.json'), Path('data/luxeuil/cache'))]:
